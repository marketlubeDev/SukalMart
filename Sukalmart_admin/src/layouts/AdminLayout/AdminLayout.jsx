  ...
  return (
    <div className="h-screen bg-gray-50">
      ...
      <aside
        ...
        aria-label="Sidebar"
      >
        <div className="h-full pl-6 pr-4 pb-4 overflow-y-auto" style={{ backgroundColor: '#6D0D2605' }}>
          {/* Logo Section */}
              ...
              {filteredMenuItems.length > 0 ? (
                ...
                onClick={() => item.path && navigate(item.path)}
                    ...
                    >
                          ...
                          isActiveRoute(item.path) ? '' : 'text-gray-400 group-hover:text-gray-600'
                        }`}>
                          {React.cloneElement(item.icon, {
                            style: {
                              ...
                              filter: isActiveRoute(item.path) 
                                ...
                                : item.icon.props.style?.filter || 'brightness(0) saturate(100%) invert(47%) sepia(8%) saturate(1077%) hue-rotate(185deg) brightness(95%) contrast(88%)'
                            }
                          })}
                      </div>
                      <span className="flex-1 text-left">{item.name}</span>
                      {item.count && (
                        <div className="px-2 py-1 text-xs font-medium rounded-full min-w-[20px] text-center"
                        style={{ backgroundColor: '#6D0D261F', color: '#6D0D26' }}>
                          {item.count}
                        </div>
                      )}
                    </button>
